...
public class Principal {
 ...
 case "5":
    ...
    ArrayList<Author> auuthorsList = library.getAuthors();
    if (auuthorsList.isEmpty()) {
        System.out.println("No hay autores en la biblioteca.\n");
        break;
    } else {
        System.out.println("Lista de autores:");
        for (int i = 0; i < auuthorsList.size(); i++) {
            System.out.println((i+1) + ". " + auuthorsList.get(i).getProfile().getName() + " " + auuthorsList.get(i).getProfile().getLastName());
        }
        System.out.println();
    }
    ...
    if (authorIndex < 0 || authorIndex >= auuthorsList.size()) {
        System.out.println("Error: El número de autor especificado no es válido.");
        break;
    }
    
    Author sselectedAuthor = auuthorsList.get(authorIndex);
    ...
    Book newBook = new Book(isbn, title, sselectedAuthor, publishDate, true);
    ...
    try {
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
        birthdat = dateFormat.parse(birthdateSt);
    } catch (ParseException e) {
        System.out.println("Error: Formato de fecha incorrecto. Use el formato dd/mm/yyyy.\n");
        break;
    }
    
    Profile clientProfile = new Profile(clientName, clientLastName, birthdat);
    Client newClient = new Client(clientProfile, new ArrayList<>());
    library.createClient(newClient);
